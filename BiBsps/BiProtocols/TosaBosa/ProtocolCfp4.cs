...
{
    public class ProtocolCfp4 : BaseProtocol
    {
        private readonly int _delayTime;

        
        public ProtocolCfp4(int boardNum, BaseCom com, DriverLog log) : base(boardNum, com, log)
        {
            _delayTime = 10000;
        ...
        public override void SelectProductType()
        {
            ...
            do
            {
                ...
                byte[] response = this.QueryProduct(cmd, _delayTime);
        ...
        public override void InitBurnInMode()
        {
            ...
            do
            {
                var cmd = new byte[] { 0x01, 0xaf, 0x10, 0x00 };
                byte[] response = this.QueryProduct(cmd, _delayTime);
                if (response.Length == espectLength)
                {
                    cmd = new byte[] { 0x01, 0x8f, 0x10, 0x00 };
                    response = this.QueryProduct(cmd, _delayTime);
                    if (response.Length == espectLength)
                        return;
                }
                retry++;
